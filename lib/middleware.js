...
/*jslint unparam: true */
module.exports = function (options, spriteOptions) {
    if (!options.src) {
    ...
    if (spriteOptions) {
        console.log(chalk.red('middleware arguments[2] options is deprecated.'));
        options.padding = options.padding || spriteOptions.padding || 0;
        options.verbose = options.verbose || spriteOptions.verbose || false;
    }
